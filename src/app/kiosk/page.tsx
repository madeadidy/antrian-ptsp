...
export default function KioskPage() {
  ...
  // Langkah 2: Eksekusi simpan ke DB dan cetak langsung
  async function handlePrintQueue() {
    ...
    try {
      ...
      const { count } = await supabase.from("queues").select("*", { count: "exact", head: true }).eq("service_id", selectedService.id).gte("created_at", `${today}T00:00:00.000Z`);
      ...
      const { error } = await supabase.from("queues").insert([{ queue_number: queueNumber, service_id: selectedService.id, status: "waiting" }]);
      ...
      // Paksa React memperbarui template struk kertas saat ini juga secara sinkron
      flushSync(() => {
        setReceipt(newReceipt);
      });

      // Pemicu cetak dijalankan sinkron (Aman dari Browser Locker)
      window.print();

      // ─── FIX UTAMA UNTUK RUNTIME ERROR CLIENT-SIDE EXCEPTION ───
      // Berikan jeda makrotask 100ms agar Next.js selesai merestorasi style layar
      // sebelum kita menghapus komponen cetak dari Virtual DOM.
      setTimeout(() => {
    ...
    <div className="min-h-screen bg-slate-100 p-6 flex flex-col items-center justify-center print:bg-white print:p-0">
      ...
      {selectedService && !receipt && (
        <div className="fixed inset-0 bg-slate-900/60 z-50 flex items-center justify-center p-4 print:hidden backdrop-blur-sm">
          <div className="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-md border border-slate-100 text-center animate-in fade-in zoom-in-95 duration-150">
            ...
            <p className="text-slate-500 text-sm mb-8">Apakah Anda yakin ingin mencetak nomor antrian untuk layanan ini? Silahkan klik tombol hijau di bawah.</p>

            <div className="flex gap-4">
              <button disabled={loading} onClick={() => setSelectedService(null)} className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold py-3.5 rounded-xl transition active:scale-95">
                Kembali
              </button>
              <button disabled={loading} onClick={handlePrintQueue} className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3.5 rounded-xl shadow-md transition active:scale-95">
      ...
      {receipt && (
        <div className="hidden print:block print:w-[76mm] print:h-[60mm] print:max-h-[60mm] text-black font-mono text-xs mx-auto text-center leading-tight overflow-hidden">
          ...
          <div className="text-4xl font-black my-2 tracking-tight">{receipt.queueNumber}</div>

          <div className="border-t border-b border-black py-1 my-1 font-bold uppercase text-[10px]">{receipt.serviceName}</div>
