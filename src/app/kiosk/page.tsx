...
export default function KioskPage() {
  ...
  // State untuk alur validasi cetak
  ...
  return (
    <div className="min-h-screen bg-slate-100 p-6 flex flex-col items-center justify-center print:bg-white print:p-0">
      ...
      {/* ─── MODAL DIALOG VALIDASI CETAK (POP-UP KONFIRMASI) ─── */}
          ...
          <div className="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-md border border-slate-100 text-center animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="flex gap-4">
              <button
                ...
                className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold py-3.5 rounded-xl transition active:scale-95 disabled:opacity-50"
              >
                Kembali
              </button>
              <button
                ...
                className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3.5 rounded-xl shadow-md transition active:scale-95 disabled:opacity-50"
              >
                {loading ? 'Mencetak...' : 'Cetak Antrian'}
              </button>
            </div>
          </div>
        </div>
      ...
      {/* ─── TEMPLATE STRUK PRINTER THERMAL (HANYA MUNCUL SAAT DI-PRINT) ─── */}
      {receipt && (
